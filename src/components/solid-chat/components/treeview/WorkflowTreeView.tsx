...
export const WorkflowTreeView = (props: WorkflowTreeViewProps) => {
  ...
  const [isPanelExpanded, setIsPanelExpanded] = createSignal(
    props.initiallyExpanded !== false,
  )
  const [copied, setCopied] = createSignal(false)
  const [treeData, setTreeData] = createSignal<any[]>([])
  const [executionStatus, setExecutionStatus] = createSignal<string | null>(
    null,
  )
  ...
  const getExecutionStatus = (executionTree: any[]) => {
    ...
    if (statuses.includes('RUNNING') || statuses.includes('INPROGRESS'))
      return 'INPROGRESS'
  ...
  const buildTreeData = (nodes: WorkflowNode[]) => {
    ...
    iterationGroups.forEach((iterationMap, parentId) => {
      iterationMap.forEach((_: string[], iterationIndex: number) => {
        ...
        const firstChildId = iterationMap.get(iterationIndex)[0]
        ...
        const childNodes = iterationMap
          .get(iterationIndex)
          .map((id: string) => nodeMap.get(id))
        const iterationStatus = childNodes.some(
          (n: any) => n.status === 'ERROR',
        )
          ? 'ERROR'
          : childNodes.some(
                (n: any) => n.status === 'INPROGRESS' || n.status === 'RUNNING',
              )
            ? 'INPROGRESS'
        ...
        iterationMap.get(iterationIndex).forEach((childId: string) => {
            ...
            for (const prevNodeId of node.previousNodeIds) {
              ...
              nodeMap.forEach((potentialParent) => {
                if (
                  ...
                  potentialParent.data?.iterationIndex ===
                    node.data?.iterationIndex &&
                  potentialParent.data?.parentNodeId ===
    ...
    const sortChildrenNodes = (node: any) => {
      if (node.children && node.children.length > 0) {
        ...
        node.children.sort((a: any, b: any) => {
          ...
          const aIsIteration =
            a.data?.name === 'iterationAgentflow' || a.data?.isVirtualNode
          // Check if b is an iteration node
          const bIsIteration =
            b.data?.name === 'iterationAgentflow' || b.data?.isVirtualNode
  ...
  const copyToClipboard = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        JSON.stringify(nodeDetails.data || {}, null, 2),
      )
      setCopied(true)
  ...
  const renderNode = (node: any) => {
    ...
    const isIterationNode =
      node.name === 'iterationAgentflow' || node.data?.isVirtualNode
    ...
    const nodeLabel = (
      <div class="flex items-center">
        ...
        <span class={isIterationNode ? 'iteration-node-label' : ''}>
          {node.label}
        </span>
      </div>
    )
    ...
    const statusIcon = (
      <div class="status-icon">{getStatusIcon(node.status)}</div>
    )
  ...
  return (
    <div
      class={`mb-2 ml-2 overflow-hidden rounded-lg border shadow-sm ${props.class || ''}`}
      style={{
    ...
    >
      {/* Collapsible header */}
      <div
        class="flex cursor-pointer items-center justify-between p-4 transition-colors duration-150 hover:bg-gray-100"
        onClick={togglePanel}
        style={{
      ...
      <Show when={isPanelExpanded()}>
        <div class="border-t">
          ...
          {selectedNode() && (
            <div
              class="node-details-panel mx-4 mb-4 rounded border p-4"
              style={{
                'background-color': 'rgba(0,0,0,0.03)',
              }}
            >
              <div class="mb-3 flex flex-col items-start justify-between md:flex-row">
                <div class="mb-2 flex items-center md:mb-0">
                  <span class="status-icon mr-2">
                    {getStatusIcon(
                      getSelectedNodeDetails()?.status || 'PENDING',
                    )}
                  </span>
                  <h3 class="font-medium">
                    <span class="font-bold">
                      {getSelectedNodeDetails()?.nodeLabel}
                    </span>
                  </h3>
                </div>
                <button
              ...
              <div
                class="json-viewer max-h-60 overflow-auto rounded p-2 font-mono text-xs"
                style={{
